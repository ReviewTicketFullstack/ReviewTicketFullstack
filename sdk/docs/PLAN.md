...
**게이트 (공통 + 아래)**
- 인수 테스트 P2 열 (P1 전체 + AC-30, 31, 32, 71) — ✅ 61건 통과
- **AC-32 가 이 Phase 의 핵심이다** — 두 백엔드의 `similarity` 차이 ≤ 1e-5,
  `matched`/`matchedKey` 완전 일치. 통과 못 하면 진행하지 않는다(BC-2)
  — ✅ 실측 차이 **3.428e-09**, matched·matchedKey·표본별 점수 전부 일치
- **AC-30 계약 테스트를 실제 python 을 띄워 돌린다.** torch 코사인 vs Java 코사인
  — ✅ 20쌍 최대 절대오차 **1.881e-08** (한계 1e-5, 여유 500배 이상)
- 수동: 실제 리뷰 사진 20쌍을 양쪽 백엔드로 돌려 0.80 근처에서 판정이 뒤집히는
  사례가 0건인지 확인 — ⏳ **미실시.** 아래 참고

**측정값 (SPEC §7 기록용)**

| 항목 | 실측 | 한계 |
|---|---|---|
| AC-30 torch ↔ Java 코사인 (20쌍 최대) | 1.881e-08 | 1e-5 |
| 대조군 torch ↔ 순수 파이썬 | 1.881e-08 | — |
| AC-32 pairwise ↔ embedding 판정 | 3.428e-09 | 1e-5 |

대조군을 함께 재는 이유 — 이 개발 환경은 Anaconda 와 torch 의 OpenMP 충돌을
우회 플래그로 넘기고 있는데, 그 플래그는 '조용히 틀린 결과'를 낼 수 있다고
경고된 것이다. torch 값이 torch 를 거치지 않은 계산과 일치한다는 것이
그 우회가 수치를 건드리지 않았다는 증거가 된다.

**아직 실제 DINOv2 로 확인하지 못했다.** 위 수치는 결정론적 스텁 벡터로 잰
것이다. 코사인은 벡터가 어느 모델에서 나왔는지와 무관한 계산이라 결론이
달라질 이유는 없지만, 실제 임베딩 분포에서 재 본 것은 아니다. 그래서
**기본 백엔드는 아직 `pairwise` 다** — 실서버 확인 뒤에 바꾼다.
